...
cmd:option('--lrdecaystart', 1, 'epoch after which learning rate decay starts')
cmd:option('--lrdecay', 1, 'multiply learning rate by this factor every epoch')
...
if not opt.silent then
   ...
   print(lm)
   print("Number of parameters: " .. lm:getParameters():nElement())
end
...
while opt.maxepoch <= 0 or epoch <= opt.maxepoch do
   ...
   elseif epoch >= opt.lrdecaystart and opt.lrdecay ~= 1 then
      opt.lr = opt.lr * opt.lrdecay
   else
   ...
   for i, inputs, targets in testset:subiter(opt.seqlen) do
      targets = targetmodule:forward(targets)
      local outputs = lm:forward(inputs)
      local err = criterion:forward(outputs, targets)
      sumErr = sumErr + err
